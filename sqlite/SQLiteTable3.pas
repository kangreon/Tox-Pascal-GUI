...
type
  ...
  public
    ...
    function GetTable(const SQL: Ansistring; const Bindings: array of const)
      : TSQLiteTable; overload;
    ...
    function GetUniTable(const SQL: Ansistring; const Bindings: array of const)
      : TSQLiteUniTable; overload;
    function GetTableValue(const SQL: Ansistring): int64; overload;
    function GetTableValue(const SQL: Ansistring;
      const Bindings: array of const): int64; overload;
    ...
    function Backup(TargetDB: TSQLiteDatabase; targetName: Ansistring;
      sourceName: Ansistring): integer; Overload;
    ...
    if iResult <> SQLITE_OK then
      ...
      begin
        ...
        raise ESQLiteException.CreateFmt
          ('Failed to open database "%s" : unknown error', [FileName]);
...
{$WARNINGS OFF}
procedure TSQLiteDatabase.BindData(Stmt: TSQLiteStmt;
...
begin
  ...
  begin
    case Bindings[I].VType of
      vtString, vtAnsiString, vtPChar, vtWideString, vtPWideChar, vtChar,
        vtWideChar:
        begin
          case Bindings[I].VType of
            vtString:
            ...
            vtPWideChar:
              begin
                DataPtr :=
                  PAnsiChar(UTF8Encode(WideString(Bindings[I].VPWideChar)));
                DataSize := -1;
              end;
            vtWideString:
              begin
                DataPtr :=
                  PAnsiChar(UTF8Encode(PWideString(@Bindings[I].VWideString)^));
                DataSize := -1;
              end;
            vtChar:
              begin
            ...
            vtWideChar:
              begin
                DataPtr :=
                  PAnsiChar(UTF8Encode(WideString(Bindings[I].VWideChar)));
                DataSize := -1;
              end;
          else
      ...
      vtInteger:
        if (sqlite3_bind_int(Stmt, I + 1, Bindings[I].VInteger) <> SQLITE_OK)
        then
          RaiseError('Could not bind integer', 'BindData');
      vtInt64:
        if (sqlite3_bind_int64(Stmt, I + 1, Bindings[I].VInt64^) <> SQLITE_OK)
        then
          RaiseError('Could not bind int64', 'BindData');
      vtExtended:
        if (sqlite3_bind_double(Stmt, I + 1, Bindings[I].VExtended^) <>
          SQLITE_OK) then
        ...
        begin
          ...
          begin
            ...
            if (sqlite3_bind_blob(Stmt, I + 1, @PAnsiChar(BlobMemStream.Memory)
              [BlobMemStream.position], BlobMemStream.Size -
          ...
          begin
            ...
            if (DataPtr = nil) then
              raise ESQLiteException.Create
                ('Error getting memory to save blob');

            BlobStdStream.position := 0;
            BlobStdStream.Read(DataPtr^, DataSize);

            if (sqlite3_bind_blob(Stmt, I + 1, DataPtr, DataSize,
              @DisposePointer) <> SQLITE_OK) then
              RaiseError('Could not bind BLOB', 'BindData');
          end
          else
...
function TSQLiteDatabase.Backup(TargetDB: TSQLiteDatabase;
  targetName: Ansistring; sourceName: Ansistring): integer;
var
  pBackup: TSQLiteBackup;
begin
  pBackup := Sqlite3_backup_init(TargetDB.DB, PAnsiChar(targetName), self.DB,
    PAnsiChar(sourceName));
...
begin
  ...
  try
    ...
    if Sqlite3_Prepare_v2(DB.fDB, PAnsiChar(SQL), -1, Stmt, NextSQLStatement) <> SQLITE_OK
    then
...
begin
  if (fCols = nil) then
  begin
    raise ESQLiteException.Create('Field ' + FieldName +
      ' Not found. Empty dataset');
    exit;
  end;
...
begin
  ...
  self.fColCount := 0;
  self.fSQL := SQL;
  if Sqlite3_Prepare_v2(DB.fDB, PAnsiChar(SQL), -1, fStmt, NextSQLStatement) <> SQLITE_OK
  then
    DB.RaiseError('Error executing SQL', SQL);
  if (fStmt = nil) then
    DB.RaiseError('Could not prepare SQL statement', SQL);
...
function TSQLiteUniTable.FieldAsBlobPtr(I: cardinal;
  out iNumBytes: integer): Pointer;
begin
  iNumBytes := Sqlite3_ColumnBytes(fStmt, I);
