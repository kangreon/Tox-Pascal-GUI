...
{ *  —оздает новую запись дл€ пользовател€. ќпредел€ет тип создаваемой записи.
  *
  *  IsFriend - ѕринадлежит ли создаваемый пользователь к списку друзей
  *  Address - јдрес пользовател€
  *  Number - »спользуетс€ в случае принадлежности пользовател€ к списку
  *           друзей дл€ ассоциации с Core.
  * }
constructor TFriendItem.Create(IsFriend: Boolean; Address: TFriendAddress;
  Number: Integer);
begin
  if not Assigned(Address) then
    raise Exception.Create('Ќе использовать эту процедуру дл€ создани€ пользовател€ без Address');
